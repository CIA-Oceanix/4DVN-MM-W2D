...
with open('./data/SWeq/param_output.txt', 'w') as output_file:
...
anim_interval = 1                                           # How often to sample for time series
sample_interval = 1                                         # How often to sample for time series
# =============== Done with setting up arrays and initial conditions ===============

t_0 = datetime.datetime.now()  # For timing the computation loop
...
for t in tqdm(range(time_step, max_time_step)):
...
# ============================= Main time loop done ================================
print("Main computation loop done!\nExecution time: {:.2f} s".format(datetime.datetime.now() - t_0))
print("\nVisualizing results...")

print(len(eta_list))

dict_res = {}
dict_res['eta']  = eta_list
dict_res['u']    = u_list
dict_res['v']    = v_list
dict_res['param_string'] = param_string
import pickle
pickle.dump( dict_res, open('./data/SWeq/SWE_simulation.pkl', 'wb' ) )
